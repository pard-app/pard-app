...
export class RegisterPage implements OnInit {
    ...
    async register() {
        ...
        this.afAuth.auth
            ...
            .then((data) => {
                if (this.logo) {
                    this.afStorage.upload(`vendors/${data.user.uid}`, this.logo).then((image) => {
    ...
    async findPlace() {
        ...
        const req = {
            query: `${this.registerForm.value.title ? this.registerForm.value.title + ", " : ""}${
                this.registerForm.value.address ? this.registerForm.value.address + ", " : ""
            }${this.registerForm.value.city ? this.registerForm.value.city + ", " : ""}${this.registerForm.value.country}`,
    ...
    formatStripeConnectLink(): string {
        return `https://connect.stripe.com/express/oauth/authorize?client_id=${environment.stripeConfig.clientId}&suggested_capabilities[]=transfers&stripe_user[email]=${this.registerForm.value.email}`;
    }
